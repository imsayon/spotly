...
function StatusBadge({ status }: { status: string }) {
	...
	> = {
		WAITING: {
			color: "#f5c418",
			bg: "rgba(245,196,24,.12)",
			border: "rgba(245,196,24,.25)",
			label: "Waiting",
		},
		CALLED: {
			color: "#00cfff",
			bg: "rgba(0,207,255,.12)",
			border: "rgba(0,207,255,.25)",
			label: "Called",
		},
		SERVED: {
			color: "#1fd97c",
			bg: "rgba(31,217,124,.12)",
			border: "rgba(31,217,124,.25)",
			label: "Served",
		},
		MISSED: {
			color: "#ff4d6d",
			bg: "rgba(255,77,109,.12)",
			border: "rgba(255,77,109,.25)",
			label: "Missed",
		},
		PENDING_ACCEPTANCE: {
			color: "#a78bfa",
...
export default function MerchantDashboard() {
	...
	// Bootstrap: fetch outlets + connect Supabase Realtime
	useEffect(() => {
	...
	// Re-subscribe to realtime channel when outlet changes
	useEffect(() => {
	...
	useEffect(() => {
		...
		const consumerBase = env.NEXT_PUBLIC_CONSUMER_URL
		...
		>
				...
				<div style={{ display: "flex", alignItems: "center", gap: 12 }}>
					...
					>
						<div
							style={{
								...
								height: 7,
								borderRadius: "50%",
								background: store.wsConnected
									? "#1fd97c"
									: "rgba(255,255,255,.3)",
								boxShadow: store.wsConnected
									? "0 0 6px rgba(31,217,124,.5)"
									: "none",
							}}
						/>
					...
					{store.outlets.length > 1 && (
						<select
							...
							onChange={(e) =>
								store.setSelectedOutletId(e.target.value)
							}
							style={{
								padding: "10px 14px",
								borderRadius: 12,
			...
			>
				{[
					{
						...
						color: "#f5c418",
						icon: <Ic.Clock size={18} />,
						sub: `${pending.length} pending`,
					},
					{
						label: "Active Session",
						value: called ? `#${called.tokenNumber}` : "—",
						color: "#00cfff",
						icon: <Ic.Bell size={18} />,
						sub: called ? "Token called" : "Queue idle",
					},
					{
						label: "Completed",
						value: servedToday,
						color: "#1fd97c",
						icon: <Ic.Check size={18} />,
						sub: "Served today",
					},
					{
						label: "Bounce Rate",
						value: entries.length
							? `${Math.round((missedToday / entries.length) * 100)}%`
							: "0%",
						color: "#ff4d6d",
						icon: <Ic.TrendUp size={18} />,
						sub: `${missedToday} missed`,
					},
				].map((st) => (
					<div
						key={st.label}
				...
				>
					...
					<div style={{ flex: 1 }}>
						...
						>
							Customers scan this to join your live queue.{" "}
							{merchantProfile.spotId && (
								<span style={{ color: "#1fd97c" }}>
									ID: {merchantProfile.spotId}
								</span>
							)}
						</p>
						<button
			...
			{store.outlets.length === 0 ? (
				...
				>
					...
					<p style={{ color: "rgba(255,255,255,.3)", fontSize: 14 }}>
						Complete onboarding to create your first outlet.
					</p>
				</div>
			) : (
				<>
						...
						{entries.length === 0 ? (
							...
							>
								...
								<div style={{ fontSize: 13, marginTop: 4 }}>
									Customers who join will appear here in
									real-time
								</div>
							</div>
						) : (
								...
								<AnimatePresence>
									...
									{[
										...
										<QueueRow
											...
											entry={entry}
											onServed={() =>
												store.markServed(entry.id)
											}
											onAccept={() =>
												store.acceptEntry(entry.id)
											}
											onReject={() =>
												store.rejectEntry(entry.id)
											}
										/>
									))}
								</AnimatePresence>
