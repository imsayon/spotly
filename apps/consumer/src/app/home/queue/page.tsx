...
export default function ConsumerQueue() {
  ...
  const statusStyles: Record<string, { color: string; bg: string; border: string; icon: React.ReactNode; anim: string }> = {
    WAITING: { color: '#f5c418', bg: 'rgba(245,196,24,.07)', border: 'rgba(245,196,24,.25)', icon: <Ic.Clock />, anim: 'animate-pulse' },
    CALLED: { color: '#1fd97c', bg: 'rgba(31,217,124,.1)', border: 'rgba(31,217,124,.3)', icon: <Ic.Bell />, anim: 'animate-bounce' },
  }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <span style={{ ...s.badge(status === 'CALLED' ? 'green' : 'yellow') as React.CSSProperties, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          {status === 'CALLED' ? <Ic.Bell /> : <Ic.Clock />} {status === 'CALLED' ? 'CALLED' : 'WAITING'}
        </span>
      </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28, position: 'relative' }}>
        ...
        <div className={statusStyle.anim} style={{ width: 200, height: 200, borderRadius: '50%', margin: '0 auto', background: statusStyle.bg, border: `2px solid ${statusStyle.border}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', transition: 'all .6s', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: statusStyle.color, display: 'flex', alignItems: 'center', gap: 6 }}>{statusStyle.icon} {status === 'CALLED' ? "YOUR TURN" : "Waiting"}</div>
        </div>
      </div>
