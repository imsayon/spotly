...
export default function ConsumerHome() {
	...
	// TODO: Implement real promotional offers from backend
	const OFFERS: Array<{
		id: number
		title: string
		sub: string
	...
	const filtered = (merchants || []).filter((m) => {
		const matchCat =
			activeCat === "all" ||
			m.category?.toLowerCase()?.includes(activeCat.toLowerCase())
		const matchSearch =
				...
				>
					...
					>
						...
						<div style={{ flex: 1 }}>
							...
							>
								Enable location permission from your browser
								site settings, then retry to show your precise
								area.
							</div>
						</div>
						<button
			...
			<div style={{ position: "relative", marginBottom: 24 }}>
				...
				<input
					...
					style={{
						...
						paddingLeft: 48,
						paddingRight: 50,
						border: `1px solid ${searchFocus ? "rgba(245,196,24,.3)" : "rgba(255,255,255,.06)"}`,
						boxShadow: searchFocus
							? "0 0 0 4px rgba(245,196,24,.05)"
							: "none",
					}}
			...
			<div>
				...
				{loading ? (
					...
					>
						...
						>
							Try searching for a different area or shop
						</div>
					</div>
				) : (
					<div
							...
							>
								...
								<div style={{ flex: 1 }}>
									...
									>
										...
										<span>{m.location || "Bengaluru"}</span>
									</div>
								</div>
								<div style={{ textAlign: "right" }}>
									...
									>
										{m.estimatedWaitTime
											? `~${m.estimatedWaitTime.replace(/\s*MIN/i, "")}m`
