...
import { useLiveLocation, LiveLocationSnapshot } from "@/lib/useLiveLocation"
...
export default function ConsumerExplore() {
	...
	const prevLocationRef = useRef<LiveLocationSnapshot | null>(null)
	...
	const isConnectableMerchant = (merchant: any) => {
		return (
			merchant?.id &&
			!String(merchant.id).startsWith("demo-") &&
			!String(merchant.id).startsWith("osm-")
		)
	}
	...
	const toggleFav = async (merchant: any, e?: React.MouseEvent) => {
		...
		if (!outletId) {
			addToast("No outlet available to favorite", "info")
			return
		}
	...
	return (
		<div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
			...
			>
				<Ic.MapPin />{" "}
				{locLoading ? "Detecting your location..." : label}
			</p>
			...
			{isDenied && (
				<div
					style={{
						...
						padding: "12px 14px",
						marginBottom: 14,
						border: "1px solid rgba(245,196,24,.28)",
						background: "rgba(245,196,24,.06)",
					}}
				>
					<div
						style={{
					...
					>
						<div style={{ color: "#f5c418", marginTop: 2 }}>
							<Ic.MapPin />
						</div>
						<div style={{ flex: 1 }}>
							<div
							...
							>
								Open browser site settings for this app, allow
								location access, and retry.
							</div>
						</div>
						<button
							type="button"
					...
					{selected && (
						...
						>
							<div
								...
								onClick={() => openMerchant(selected)}
							>
								<div style={{ fontSize: 26, color: "#f5c418" }}>
									<Ic.Store />
								</div>
								<div style={{ flex: 1 }}>
									<div
									...
									>
										{isConnectableMerchant(selected)
											? `${selected.outlets?.length || 0} branches`
											: "Discovery result"}{" "}
										· {selected.category}
									</div>
								</div>
				...
				>
							...
							<div style={{ flex: 1 }}>
								...
								<div style={{ display: "flex", gap: 7 }}>
									...
									>
										<Ic.Clock size={12} />{" "}
										{m.estimatedWaitTime ||
											(m.currentQueueDepth != null
												? `${m.currentQueueDepth} in Q`
												: "—")}
									</span>
									<span
										style={{
					...
					{merchants.length === 0 && (
						...
						>
							...
							>
								We couldn't find any active Spotly merchants in
								this area.
							</p>
						</div>
					)}
