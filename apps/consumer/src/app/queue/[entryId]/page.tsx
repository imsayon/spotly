		...
		const fetchData = async () => {
			try {
				...
				const waitingAhead = queueEntries.filter(
					(e: any) =>
						e.status === "WAITING" &&
						e.tokenNumber < currentEntry.tokenNumber,
				).length
	...
	useEffect(() => {
		...
		const initSocket = async () => {
			...
			const onQueueUpdate = (payload: any) => {
				...
				const updated = payload.entries.find(
					(e: any) => e.id === entryId,
				)
				...
				const waitingAhead = payload.entries.filter(
					(e: any) =>
						e.status === "WAITING" &&
						e.tokenNumber <
							(updated?.tokenNumber ?? entry.tokenNumber),
				).length
				setAhead(waitingAhead)
			}
			...
			const onEntryUpdate = (payload: {
				...
				if (payload.entryId === entryId) {
					setEntry((prev) =>
						prev
							? { ...prev, status: payload.status as any }
							: null,
					)
				}
			}
				...
				<AnimatePresence mode="wait">
					{isCalled ? (
						...
						>
							...
							>
								The merchant will accept your request before
								your token enters the live queue.
							</p>
						</motion.div>
					) : isWaiting ? (
						<motion.div
						...
						>
							...
							>
								You cancelled your reservation. You can rejoin
								the queue anytime.
							</p>
						</motion.div>
					) : (
