...
export default function ConsumerMerchantPage() {
	...
	const handleJoin = async (outletId: string) => {
		...
		if (myEntry) {
			...
			if (
				!confirm(
					"You are already in another queue. Leave that and join this one?",
				)
			)
				return
			try {
		...
		>
			...
			<div style={s.banner}>
				<Orb
					...
					y="-10%"
					size="80%"
					color="rgba(245,196,24,.1)"
					anim="orb1 15s infinite"
				/>
				<Orb
					x="60%"
					y="40%"
					size="60%"
					color="rgba(255,99,22,.05)"
					anim="orb2 20s infinite"
				/>
				<div
					style={{
						position: "relative",
			...
			<div style={{ marginBottom: 44 }}>
				...
				>
					{merchant.description ||
						"Welcome to our premium outlet. Join the queue digitally and save your time."}
				</p>
			</div>
				...
				>
					...
					{outlets.length === 0 && (
						...
						>
							...
							>
								No active outlets for this merchant yet.
							</p>
						</div>
					)}
				</div>
			...
			>
				...
				>
					<div style={{ color: "#f5c418", marginBottom: 12 }}>
						<Ic.Bell />
					</div>
					<div
						style={{
					...
					>
						Instant push notifications when your turn is
						approaching.
					</div>
				</motion.div>
				<motion.div
				...
				>
					<div style={{ color: "#1fd97c", marginBottom: 12 }}>
						<Ic.Shield />
					</div>
					<div
						style={{
					...
					>
						Verified tokens ensure a fair and organized experience.
					</div>
				</motion.div>
			</div>
		</motion.div>
