...
export class IntegrationService {
	...
	async geocode(
		...
		try {
			...
			if (response.data && response.data.length > 0) {
				...
				this.logger.log(
					`Geocoding result found: ${result.displayName} (${result.lat}, ${result.lon})`,
				)
				return result
			}
	...
	async fetchShops(
		...
		this.logger.log(
			`Fetching shops at (${lat}, ${lon}) with radius ${radius}m`,
		)
		try {
			const categoryQuery = this.resolveCategoryQuery(
				category,
				radius,
				lat,
				lon,
			...
			return elements
				...
				.map((el: any) => {
					...
					return {
						...
						// OSM discovery merchants don't have real ratings or wait times
						rating: undefined,
