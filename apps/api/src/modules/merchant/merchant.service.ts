...
export class MerchantService {
	...
	async create(userId: string, data: Partial<Merchant>): Promise<Merchant> {
		...
		if (existing) {
			this.logger.log(
				`Merchant already exists for user: ${userId}. Updating profile.`,
			)
		...
		try {
			...
			if (!user) {
				this.logger.error(
					`User record ${userId} not found in database.`,
				)
			...
			if (err?.code === "P2002") {
				// Concurrent creation race — retrieve the winner
				const existing = await this.findByUser(userId)
	...
	async findAll(
		...
		const maxDistanceKm = Number.isFinite(radiusMeters)
			? (radiusMeters as number) / 1000
			: undefined

		return merchants
			.map((merchant) => {
				const candidates = [
					Number.isFinite(merchant.lat) &&
					Number.isFinite(merchant.lng)
						? {
			...
			.filter(
				(merchant) =>
					...
					merchant.distanceKm <= maxDistanceKm,
			)
			.sort(
				(a, b) =>
	...
	async updateProfile(
		...
		try {
			...
			if (!merchant) {
				this.logger.error(
					`Profile update failed: No merchant found for user ${userId}`,
				)
			...
			const spotId =
				merchant.spotId ||
				`SPOT-${crypto.randomBytes(3).toString("hex").toUpperCase()}`
