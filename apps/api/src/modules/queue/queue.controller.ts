...
export class QueueController {
	...
	async getHistory(
		...
		const data = await this.queueService.getHistory(
			user.uid,
	...
	async getEntry(
		...
		if (data.userId !== user.uid) {
			const merchant = await this.merchantService
				.findByUser(user.uid)
				.catch(() => null)
	...
	@Get(":outletId")
	async getQueue(@Param("outletId", ParseUUIDPipe) outletId: string) {
