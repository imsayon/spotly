	...
	implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
		...
		server.use(async (socket, next) => {
			try {
				if (!this.authService.isFunctional) {
					return next(
						new Error("Unauthorized: Auth service unavailable"),
					)
				}
	...
	async handleJoinOutlet(
		...
		if (!isMerchant && !isParticipant) {
			...
			this.logger.warn(
				`Client ${client.id} denied room outlet:${data.outletId}`,
			)
			return
		}
	 ...
	 */
	async emitTokenCalled(
		outletId: string,
		payload: Omit<TokenCalledPayload, "userId">,
	): Promise<void> {
	 ...
	 */
	async emitEntryUpdate(
		outletId: string,
		payload: { entryId: string; status: string },
	): Promise<void> {
		this.server.to(`outlet:${outletId}`).emit("entry_update", payload)
	}

	async emitOutletStatus(
		outletId: string,
		payload: { outletId: string; isActive: boolean; isOpen?: boolean },
	): Promise<void> {
		this.server
			.to(`outlet_status:${outletId}`)
			.emit("outlet_status", payload)
		this.server.to(`outlet:${outletId}`).emit("outlet_status", payload)
	}

	async notifyOutletStatusChange(
		outletId: string,
		isOpen: boolean,
	): Promise<void> {
		await this.emitOutletStatus(outletId, {
			outletId,
			isActive: isOpen,
			isOpen,
		})
